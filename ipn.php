...
$sandbox = get_option("efs_use_sandbox");
...
$txn_id = $_POST['txn_id'];
//error_log("START: ".$txn_id." -->".$item_number);

$sender_address = strtolower(get_option('efs_paypal_email'));
...
if($fp)
{
	...
	while (!feof($fp))
	{
		...
		if (strcmp ($res, "VERIFIED") == 0)
		{
			...
			elseif(strtolower($receiver_email) != strtolower(get_option("efs_paypal_email")))
			{
				$status = "invalid receiver email: ".$receiver_email;
			}
			elseif(floatval($payment_amount) != floatval(get_post_meta($post->ID, "efs_price", true)))
			{
				$status = "invalid price: ".$payment_amount;
			}
			elseif($payment_currency != get_option('efs_currency_code'))
			{
			...
			if(empty($txn))
			{
				...
				$sql = "INSERT INTO {$wpdb->prefix}easyfileshop
						(postid,
						hash,
						txn_id,
						price,
			...
			else
			{
				...
				$sql = "UPDATE {$wpdb->prefix}easyfileshop
						SET postid = %d,
						price = %f,
						status = %s,
						ipn_date = NOW(),
			...
			if($status == "VERIFIED")
			{
				//error_log("MAIL: ".$txn_id." ".$status);
				
				$blogname = get_option('blogname');
				$downloadlink = get_option('efs_shortlink') ? get_option('siteurl')."/efs/".$hash : WP_PLUGIN_URL."/easyfileshop/download.php?h=".$hash;
				
				$mail_From = "From: Shop <".$sender_address.">";
				$mail_Subject = get_option('efs_emailsubject');
				...
				$mail_Body = str_replace($search, $replace, get_option('efs_emailtext'));
